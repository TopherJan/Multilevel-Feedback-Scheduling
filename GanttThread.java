...
public class GanttThread extends Thread {
  ...
  public GanttThread(ArrayList<Integer> arr, ArrayList<Integer> t){
		this.start();
		this.arr = arr;
		this.t = t;
		...
		for(int x = 0; x < arr.size(); x++){
			try{
				while(index != t.size()){
					...
					MLFQFrame.addComponent(MLFQFrame.ganttPanel, timeLabel[index], distance*t.get(index),65, 16, 75);
					index++;
				}
				...
				MLFQFrame.addComponent(CPUFrame.ganttPanel, CPUFrame.processLabel[x], distance,20, 16, 75);
				distance += 18;
				Thread.sleep(100);
			}catch(Exception e){}
	    }
